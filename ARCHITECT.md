...
### Example 1: Designing HAR Capture Component
**User Query:** "Design the Chrome extension HAR capture component"

**Architecture Output Includes:**
- Capture lifecycle (start → intercept → collect → export)
- Network interception strategy (chrome.devtools.network API)
- Data structure for in-memory storage
- HAR format builder algorithm
- Export mechanism (HTTP POST to local service)
- Handle binary data (base64 encoding)
- Performance considerations (memory usage, filtering)

### Example 2: Correlation Engine Architecture
**User Query:** "How should we architect the dynamic data correlation engine?"

**Architecture Output Includes:**
- Supported pattern types (JSON path, XPath, Regex, headers, cookies)
- Correlation rule format and validation
- Extraction algorithm
- Replacement strategy
- Rule chaining (dependent correlations)
- Performance optimization (caching, compiled patterns)
- Testing approach (unit tests for each pattern type)

### Example 3: Load Testing Distribution
**User Query:** "Design distributed replay for load testing 1000 concurrent users"

**Architecture Output Includes:**
- Orchestrator node (coordination, distribution)
- Worker node scaling (how many workers needed)
- HAR distribution strategy
- Load balancing (round-robin, weighted)
- Results aggregation
- Real-time monitoring & metrics
- Failure handling (worker node crashes)
...
- Major feature architecture (capture, replay, distribution)
- Technology selection decisions
- Performance optimization planning
- Integration with UiPath products
- Scaling strategies
- Security & compliance design

❌ **Don't use for:**
- Simple feature requests ("add a button")
...
- **UiPath Orchestrator API**: https://docs.uipath.com/orchestrator/latest/api/swagger
- **HTTP Caching**: https://developer.mozilla.org/en-US/docs/Web/HTTP/Caching
- **Correlation Patterns**: SPECIFICATION.md in this repository
- **Apollo Design System**: apollo-prime/ directory in this repository
