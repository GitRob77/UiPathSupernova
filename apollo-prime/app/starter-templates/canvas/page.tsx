...
import { usePropertiesPanel } from "@/components/custom/properties";
import { widgetShowcaseSchema } from "@/components/custom/properties";
...
import { TerminalBottomPanel } from "@/components/custom/terminal-bottom-panel";
import {
  ActivityFlowList,
...
export default function CanvasDemoPage() {
  ...
  const [bottomPanelMode, setBottomPanelMode] = useState<"debug" | "terminal" | null>(null);
  const debugOpen = bottomPanelMode === "debug";
  ...
  /* --- Canvas content: flow list when no tabs, workspace when tabs open --- */
  const canvasContent =
    tabs.length === 0 ? (
      <ActivityFlowList
    ...
    <CanvasLayout
      ...
      bottomPanel={
        <>
          ...
          />
          <TerminalBottomPanel open={terminalOpen} />
        </>
      }
      statusBar={{
        solutionName: "Solution 5",
        onDebugClick: () =>
          setBottomPanelMode((prev) => (prev === "debug" ? null : "debug")),
        debugActive: debugOpen,
        onTerminalClick: () =>
          setBottomPanelMode((prev) => (prev === "terminal" ? null : "terminal")),
