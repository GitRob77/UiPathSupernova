...
export default function TerminalPanelHomePage() {
  /* --- Tab state --- */
  const [tabs, setTabs] = useState<CanvasTab[]>([]);
  const [activeTabId, setActiveTabId] = useState<string | undefined>();
  const [bottomPanel, setBottomPanel] = useState<"debug" | "terminal" | null>("terminal");

  const togglePanel = (panel: "debug" | "terminal") => {
    ...
    <CanvasLayout
      ...
      bottomPanel={
        bottomPanel === "debug" ? (
          ...
          <TerminalBottomPanel open />
        ) : null
      }
      statusBar={{
        solutionName: "Solution 5",
        onDebugClick: () => togglePanel("debug"),
        debugActive: bottomPanel === "debug",
        onTerminalClick: () => togglePanel("terminal"),
        terminalActive: bottomPanel === "terminal",
      }}
    >
      <CanvasWorkspace
        tabs={tabs}
